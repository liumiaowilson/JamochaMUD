 ...
 *
 * This code has been modified for more MUD-specific uses
 * 
 * --LICENSE NOTICE--
 ...
 *
 * @version $Id: TelnetProtocolHandler.java,v 1.3 2010/05/11 02:10:09 jeffnik
 * Exp $
...
public abstract class TelnetProtocolHandler {
     ...
     */
    public final static String ID = "$Id: TelnetProtocolHandler.java,v 1.3 2010/05/11 02:10:09 jeffnik Exp $";
    /**
     * debug level
     */
     ...
     */
    private transient byte[] tempbuf = new byte[0];
    /**
     * the data sent on pressing <RETURN> \n
     */
    private transient byte[] crlf = new byte[2];
    /**
     * the data sent on pressing <LineFeed> \r
     */
    private transient byte[] cr = new byte[2];
    /**
     * Variable tracking if we have compressed input/output: MCCP v2
     */
    private transient boolean compression2 = false;
    /**
     * Variable tracking if we are using MXP
     */
     ...
     * was closed or some other problem occurred.
     */
    public void reset() {
        neg_state = 0;
     ...
     */
    private transient byte neg_state = 0;
    /**
    ...
    /**
     * compress data as inStream MCCP v1 specification (Not supported)
     */
    private static final byte TELOPT_MCCP = (byte) 85;
    /**
     * compress data as inStream MCCP v2 specification
     */
    private static final byte TELOPT_MCCP2 = (byte) 86;
    /**
     * Support MXP (MUD eXtension Protocol)
     */
    private static final byte TELOPT_MXP = (byte) 91;
     ...
     * @throws java.io.IOException
     */
    public void sendTelnetControl(final byte code)
            throws IOException {
        byte[] b = new byte[2];
    ...
    public void setWindowSize(final int columns, final int rows)
            throws IOException {
                    logger.debug("sending NAWS");
        

        if (receivedDX[TELOPT_NAWS] != DO) {
            logger.debug("not allowed to send NAWS? (DONT NAWS)");
            return;
        }
        write(IAC);
        write(SB);
     ...
     */
    private void handle_sb(final byte type, final byte[] sbdata)
            throws IOException {
     ...
     * necessary actions are taken according to the telnet protocol. See
     * <A HREF="RFC-Telnet-URL">RFC-Telnet</A>
     *
    ...
    public int negotiate(byte nbuf[])
            throws IOException {
        final int count = tempbuf.length;
                ...
                case STATE_IACWILL:
                    switch (b) {
                        ...
                        case TELOPT_MXP:
                                ...
                                logger.debug("TelnetProtocolHandler: MXP protocol request.");
                ...
                case STATE_IACSBDATA:
                    ...
                    switch (b) {
                        case IAC:
                            ...
                            break;
                        default:
                            final byte[] xsb = new byte[sbbuf.length + 1];
